...
namespace Saraff.Twain.DS.Capabilities {
    ...
    internal abstract class DeviceDataSourceCapability<TValue>:OneDataSourceCapability<TValue> {

        protected DevicePropertiesAttribute DeviceProperties {
            get {
                foreach(DevicePropertiesAttribute _attr in this.DS.GetType().GetCustomAttributes(typeof(DevicePropertiesAttribute), false)) {
                    return _attr;
                }
                throw new InvalidOperationException("Не указан DevicePropertiesAttribute.");
            }
        }
    }
}
