...
namespace Saraff.Twain.DS.Capabilities {
    ...
    [DataSourceCapability(TwCap.IXferMech, TwType.UInt16, SupportedOperations=TwQC.Get|TwQC.GetCurrent|TwQC.GetDefault|TwQC.Set|TwQC.Reset, Get=TwOn.Enum)]
    internal sealed class XferMechDataSourceCapability:EnumDataSourceCapability<TwSX> {
        private Collection<TwSX> _values;

        #region EnumDataSourceCapability

        protected override Collection<TwSX> CoreValues {
            get {
                return this._values??(this._values=this.DS.SupportedXferMechs.ToCollection());
            }
            set {
                this._values=value;
                if(!this._values.Contains(TwSX.Native)) {
                    this._values.Add(TwSX.Native);
                }
                this.DefaultIndexCore=this._values.Find(val => val==TwSX.Native);
            }
        }
