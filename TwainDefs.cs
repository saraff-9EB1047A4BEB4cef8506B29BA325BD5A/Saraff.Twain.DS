...
namespace Saraff.Twain.DS {
    ...
    [StructLayout(LayoutKind.Sequential, Pack=2, CharSet=CharSet.Ansi)]
    public sealed class TwIdentity {									// TW_IDENTITY
        private uint _id;
        private TwVersion _version;
        private ushort _protocolMajor;
        private ushort _protocolMinor;

        [MarshalAs(UnmanagedType.U4)]
        private TwDG _supportedGroups;
        private TwStr32 _manufacturer;
        private TwStr32 _productFamily;
        private TwStr32 _productName;

        private TwIdentity() {
        }

        internal TwIdentity(uint id, TwVersion version, ushort protocolMajor, ushort protocolMinor, TwDG supportedGroups, TwStr32 manufacturer, TwStr32 productFamily, TwStr32 productName) {
            this._id=id;
    ...
    internal struct TwVersion {									// TW_VERSION
        private ushort _majorNum;
        private ushort _minorNum;

        [MarshalAs(UnmanagedType.U2)]
        private TwLanguage _language;

        [MarshalAs(UnmanagedType.U2)]
        private TwCountry _country;
        private TwStr32 _info;

        internal TwVersion(ushort majorNum, ushort minorNum, TwLanguage language, TwCountry country, TwStr32 info) {
            this._majorNum=majorNum;
            this._minorNum=minorNum;
    ...
    /// <summary>
    /// This structure is used to pass specific information between the entry source and the appId.
    /// </summary>
    ...
    //    /// <summary>
    //    /// This is the return code of availability of entry for extended image attribute requested.
    //    /// </summary>
    //    [MarshalAs(UnmanagedType.U2)]
    //    public TwRC ReturnCode;

    //    /// <summary>
    //    /// Contains either entry or a handle to entry.
    //    /// </summary>
    //    public IntPtr Item;
    ...
    //        Marshal.StructureToPtr(new TwExtImageInfo {
    ...
    internal class TwCapability:IDisposable {					// TW_CAPABILITY
        ...
        public TwCapability(TwCap cap, string value, TwType type) {
            ...
            this.Handle=DataSourceServices.Memory.Alloc(_twOneCustumValueSize+Marshal.SizeOf(TwTypeHelper.TypeOf(type)));
            IntPtr _ptr=DataSourceServices.Memory.Lock(this.Handle);
            try {
                Marshal.StructureToPtr(new TwOneCustumValue {
                    ItemType=type
                }, _ptr, true);
                Marshal.StructureToPtr(TwTypeHelper.CastToTw(type, value), (IntPtr)(_ptr.ToInt64()+_twOneCustumValueSize), true);
            } finally {
                DataSourceServices.Memory.Unlock(this.Handle);
            }
        }
        ...
        public TwCapability(TwCap cap, TwArray array, object[] arrayValue) {
            ...
            this.Handle=DataSourceServices.Memory.Alloc(_twArraySize+(_twItemSize*arrayValue.Length));
            IntPtr _pTwArray=DataSourceServices.Memory.Lock(this.Handle);
            try {
        ...
        public TwCapability(TwCap cap, TwEnumeration enumeration, object[] enumerationValue) {
            ...
            this.Handle=DataSourceServices.Memory.Alloc(_twEnumerationSize+(_twItemSize*enumerationValue.Length));
            IntPtr _pTwEnumeration=DataSourceServices.Memory.Lock(this.Handle);
            try {
        ...
        public void Dispose() {
            if(this.Handle!=IntPtr.Zero) {
                DataSourceServices.Memory.Free(this.Handle);
        ...
        private void _SetValue<T>(T value) {
            this.Handle=DataSourceServices.Memory.Alloc(Marshal.SizeOf(typeof(T)));
            IntPtr _ptr=DataSourceServices.Memory.Lock(this.Handle);
            try {
    ...
    //[return: MarshalAs(UnmanagedType.U2)]
    //public delegate TwRC DSEntry(TwIdentity appId, [MarshalAs(UnmanagedType.U4)]TwDG dg, [MarshalAs(UnmanagedType.U2)]TwDAT dat, [MarshalAs(UnmanagedType.U2)]TwMSG msg, IntPtr entry);
